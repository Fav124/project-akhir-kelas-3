    ...
    <div class="flex w-full lg:w-1/2 flex-col justify-center items-center p-6 sm:p-12 lg:p-24 bg-background-light dark:bg-background-dark overflow-y-auto">
        <div class="w-full max-w-md flex flex-col gap-8">
            <div class="flex flex-col gap-2">
                ...
                <p class="text-text-muted dark:text-gray-400 text-base">Selamat datang kembali! Silakan masukkan detail login Anda.</p>
            </div>
            ...
            <form method="POST" action="{{ route('login') }}" class="flex flex-col gap-5">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input class="form-input flex w-full rounded-lg border border-[#cfe7db] dark:border-gray-700 bg-surface-light dark:bg-surface-dark px-4 py-3 text-base text-text-main dark:text-white placeholder:text-text-muted dark:placeholder:text-gray-500 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors h-12 @error('email') border-error @enderror" id="email" name="email" value="{{ old('email') }}" required autofocus placeholder="user@example.com" type="email"/>
                </div>

                <div class="flex flex-col gap-2">
                    <label class="text-text-main dark:text-gray-200 text-sm font-semibold leading-normal" for="password">Password</label>
                    <div class="relative flex items-center">
                        <input class="form-input flex w-full rounded-lg border border-[#cfe7db] dark:border-gray-700 bg-surface-light dark:bg-surface-dark px-4 py-3 pr-12 text-base text-text-main dark:text-white placeholder:text-text-muted dark:placeholder:text-gray-500 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors h-12 @error('password') border-error @enderror" id="password" name="password" required placeholder="Masukkan password" type="password"/>
                    </div>
                </div>

                <div class="flex justify-between items-center">
                     <label class="inline-flex items-center">
                        <input type="checkbox" class="form-checkbox text-primary border-gray-300 rounded focus:border-primary focus:ring focus:ring-offset-0 focus:ring-primary focus:ring-opacity-50" name="remember">
                        <span class="ml-2 text-sm text-text-muted">Remember me</span>
                    </label>
                    <a class="text-primary hover:text-green-600 text-sm font-semibold hover:underline transition-colors" href="#">Lupa Password?</a>
                </div>

                <button type="submit" class="flex w-full cursor-pointer items-center justify-center rounded-lg bg-primary hover:bg-green-400 text-[#0d1b14] text-base font-bold h-12 px-6 transition-all active:scale-[0.98]">
                    Log In
                </button>
            </form>
