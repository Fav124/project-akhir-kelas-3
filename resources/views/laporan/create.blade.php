...
@section('content')
    ...
    <div class="mb-4">
        ...
        <h4 class="fw-bold mb-0">Tambah Laporan Pemeriksaan</h4>
        <small class="text-muted">Masukkan data pemeriksaan kesehatan santri</small>
    </div>
    ...
    <div class="card border-0 shadow-sm" style="max-width: 600px;">
        <div class="card-body">
            <h5 class="fw-bold mb-4">
                <i class="bi bi-file-earmark-text me-2 text-primary"></i> Form Laporan Pemeriksaan
            </h5>
            ...
            <form method="POST" action="{{ route('laporan.store') }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('tanggal_pemeriksaan') is-invalid @enderror"
                        id="tanggal_pemeriksaan" name="tanggal_pemeriksaan" value="{{ old('tanggal_pemeriksaan') }}"
                        required>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('keluhan') is-invalid @enderror" id="keluhan" rows="3"
                        placeholder="Masukkan keluhan santri" name="keluhan" required>{{ old('keluhan') }}</textarea>
                    @error('keluhan')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('suhu_tubuh') is-invalid @enderror" id="suhu_tubuh"
                        placeholder="Contoh: 36.5" name="suhu_tubuh" step="0.1" value="{{ old('suhu_tubuh') }}"
                        required>
                    @error('suhu_tubuh')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('tindakan') is-invalid @enderror" id="tindakan" rows="3"
                        placeholder="Tindakan yang diberikan" name="tindakan">{{ old('tindakan') }}</textarea>
                    @error('tindakan')
                        <div class="invalid-feedback d-block">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('status_kondisi') is-invalid @enderror" id="status_kondisi"
                        ...
                        <option value="sehat" {{ old('status_kondisi') == 'sehat' ? 'selected' : '' }}>Sehat</option>
                        <option value="sakit-ringan" {{ old('status_kondisi') == 'sakit-ringan' ? 'selected' : '' }}>Sakit
                            Ringan</option>
                        <option value="sakit-berat" {{ old('status_kondisi') == 'sakit-berat' ? 'selected' : '' }}>Sakit
                            Berat</option>
                    </select>
